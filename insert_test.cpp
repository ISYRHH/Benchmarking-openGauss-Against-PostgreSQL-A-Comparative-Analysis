...
int main()
{
    ...
    try
    {
        ...
        if(C1.is_open())
        {
            ...
            {
                nontransaction W(C1);
                tot1+=get_time(W.exec("explain analyse insert into b values ("+to_string(vec[i])+"),("+to_string(vec[i+1])+");"));
            }
        }
        ...
        if(C2.is_open())
        {
            ...
            {
                nontransaction W(C2);
                tot2+=get_time(W.exec("explain analyse insert into b values ("+to_string(vec[i])+"),("+to_string(vec[i+1])+");"));
            }
        }
        printf("Runtime of openGauss : %f\nRuntime of PostgreSQL : %f\n",tot1,tot2);
    }
