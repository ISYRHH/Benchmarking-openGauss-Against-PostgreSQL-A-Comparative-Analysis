...
int main()
{
    ...
    try
    {
        ...
        if(C1.is_open())
        {
            ...
            {
                transaction W(C1);
                tot1+=get_time(W.exec("explain analyse update a set id = -1 where id between "+to_string(vec[i])+" and "+to_string(vec[i+1])+";"));
                W.abort();
            }
        }
        ...
        if(C2.is_open())
        {
            ...
            {
                transaction W(C2);
                tot2+=get_time(W.exec("explain analyse update a set id = -1 where id between "+to_string(vec[i])+" and "+to_string(vec[i+1])+";"));
                W.abort();
            }
        }
