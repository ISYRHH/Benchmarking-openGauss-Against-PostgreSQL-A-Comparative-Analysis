...
int main()
{
    ...
    try
    {
        ...
        if(C1.is_open())
        {
            ...
            {
                nontransaction W(C1);
                tot1+=get_time(W.exec("explain analyse select * from b where id between "+to_string(vec[i])+" and "+to_string(vec[i+1])+" order by id;"));
            }
        }
        ...
        if(C2.is_open())
        {
            ...
            {
                nontransaction W(C2);
                tot2+=get_time(W.exec("explain analyse select * from b where id between "+to_string(vec[i])+" and "+to_string(vec[i+1])+" order by id;"));
            }
        }
